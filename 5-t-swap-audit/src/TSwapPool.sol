...
contract TSwapPool is ERC20 {
    ...
    //report written large literals value instead use 1e18 nonation
    ...
    function deposit(
        ...
        //report-written DeadLine is never used 
        //Impact-High If the person sets deadline as block.timestamp+30 he thinks the transaction reverts and it is safe but however the transaction does not reverts. There will be high chance of front running and user might loose some money as slippage
 //Likelihood-High 
        uint64 deadline 
    )
        external
        ...
        returns (uint256 liquidityTokensToMint)
    {
        ...
        if (totalLiquidityTokenSupply() > 0) {
            ...
            //report-written Dont need this line can be ignored
            ...
            //report-written - It would have been better if liquidityTokenToMint was called before _addLiquidityMintAndTransfer to follow CEI
        ...
        private
    {
        ...
        //report-written this is backward. This should be  -:
        // (msg.sender , wethToDeposit , poolTokensToDeposit)
        //Impact Low Protocol is going to give wrong information
        ...
        returns (uint256 outputAmount)
    {
        ...
        // report-wrritten MagicNumbers should not be literals but should be defined as constants.
    ...
    { //report written MagicNumbers should not be literals but should be constants

     //report-written written 10000 instead of 1000
     //Impact-High Users are charged fee too much
...
//report-written Where Is the natspec
    function swapExactInput(
        IERC20 inputToken, //e dai
        uint256 inputAmount, // 1000
        IERC20 outputToken, // weth
        uint256 minOutputAmount, // 10 
        uint64 deadline
    )
    //report-written This should be external to avoid gas
        public
        revertIfZero(inputAmount)
        revertIfDeadlinePassed(deadline)
        //report-written This is not being used
        //impact-low This is always gonna return 0 hence wrong output
        //Likelihood-High always the case
...
//report-written where is the natspec ???
        ...
        returns (uint256 inputAmount)
    {
        uint256 inputReserves = inputToken.balanceOf(address(this));
        uint256 outputReserves = outputToken.balanceOf(address(this));
        //report-written no slippage protection at least have max input amount
